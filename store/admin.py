...
def apply_offer_men_shirts(modeladmin, request, queryset):
    ...
    men_shirts = queryset.filter(
        category__category_name="Men", subcategory__sub_cat_name="Shirts"
    )
    ...
    if discount_offer:
        ...
        for product in offer_products:
            ...
            for variation in variations:
                offer_price = variation.price - (
                    (variation.price * discount_offer.discount.discount) / 100
                )
...
def jackets_women(modeladmin, request, queryset):
    women_jackets = queryset.filter(
        category__category_name="Women", subcategory__sub_cat_name="Jackets"
    )
    try:
...
class ProductAdmin(admin.ModelAdmin):
    ...
    actions = [
        "undelete_selected",
        apply_offer_men_shirts,
        jackets_women,
    ]  # Define a custom action to undelete products

    def undelete_selected(modeladmin, request, queryset):
        queryset.update(
            is_deleted=False
        )  # Set is_deleted to False for selected products

    undelete_selected.short_description = (
        "Undelete selected products"  # Action description
    )
