...
def product_detail(request, category_slug, Product_slug):
    ...
    try:
        ...
        pass
    # end oos
    variation_for_preselection = Variation.objects.filter(
        product=single_product, stock__gt=0
    )
    for variation in variation_for_preselection:
        if variation.color.color_name not in unique_color:
            ...
            var_size = Variation.objects.filter(
                product=single_product, color=variation.color
            )
            size = set()
    ...
    if request.method == "GET":
        ...
        try:
            ...
            variation = Variation.objects.get(
                product_id=product_id, color__color_name=color, size__size=size
            )
            offer_price = variation.offer_price
            regular_price = variation.price

            response_data = {"offer_price": offer_price, "price": regular_price}
...
def submit_review(request, product_id):
    ...
    if request.method == "POST":
        try:
            ...
            form.save()
            messages.success(
                request, "Thank you,your review has been updated", extra_tags="review"
            )
            return redirect(url)
            ...
            if form.is_valid():
                ...
                messages.success(
                    request,
                    "Thank you,your review has been submited",
                    extra_tags="review",
                )
                return redirect(url)
...
def filter_products(request):
    ...
    if size:
        ...
        if category:
            variations = variations.filter(size__in=size)
        else:
    ...
    if max_price:
        if min_price or category or size:
            variations = variations.filter(
