...
from store.models import Product,softdelete,NonDeleted,Offer,com_offers,Discount,Variation,Sub_category
...
def ad_dashboard(request):   
    ...
    completed_transaction_by_payment = completed_orders.values('payment__payment_method').annotate(
    method_count = Count('payment__payment_method')
    ) 
    transaction_type = []
    transaction_count = []
    ...
    #find  orders by date
    orders_growth=Order.objects.annotate(
    date=TruncDate('created_at')
        ).values('date').annotate(
            count_by_date=Count('id')
        ).order_by('date')
    order_date = []
    order_count = []    
    ...
    # todays orders
    today_orders = Order.objects.filter(created_at__range=
            (start_of_day,end_of_day),status='Completed').aggregate(todays_order_count=Count('order_number'))['todays_order_count'] or 0 
    
    # total orders
    total_orders = Order.objects.filter(Q(status='Completed')|Q(status='Cancelled')).aggregate(total_orders=Count('order_number'))['total_orders'] or 0
    
    # order table for payment
    orders = Order.objects.filter(status='Completed').order_by('-created_at')[:5]
    return render(request,'admins/dashboard.html', {'category_name_json': category_name_json,
        'category_count_json': category_count_json,
        'transaction_type_json':transaction_type_json,'transaction_count_json':transaction_count_json,
        'order_date_json':order_date_json,'order_count_json':order_count_json,'orders':orders
        ,'today_sales':today_sales,'total_sales':total_sales,'today_orders':today_orders,'total_orders':total_orders})
...
def sales_report(request):
    ...
    order_detail = order_detail.annotate(total_discount=F('discount_from_offers') + F('discount_from_coupons'))
     # today sale
    current_datetime = timezone.now()
    start_of_day = current_datetime.replace(hour=0, minute=0, second=0, microsecond=0)
    end_of_day = current_datetime.replace(hour=23, minute=59, second=59, microsecond=999999)
