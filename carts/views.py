        ...
        if is_cart_item_exists:
            ...
            if variation in existing_variation_list and next_url != "1":
                ...
                if item.quantity > variation.stock:
                    messages.error(
                        request,
                        f"Only {variation.stock} piece is available for this variation",
                        extra_tags="quantity",
                    )
                ...
                item = CartItem.objects.create(
                    product=product, quantity=1, user=current_user, variations=variation
                )
                item.save()
        else:
    ...
    # if the user is not authenticated
        ...
        if is_cart_item_exists:
            ...
            if variation in existing_variation_list and next_url != "1":
                ...
                # create a cart item
                item = CartItem.objects.create(
                    product=product, quantity=1, cart=cart, variations=variation
                )
                item.save()
        else:
            cart_item = CartItem.objects.create(
...
def checkout(request, total=0, quantity=0, cart_items=None):
    ...
    if request.method == "POST":
        ...
        valid_coupon = None
        try:
            coupon_name = request.POST.get(
                "coupon_code"
            )  # fetch the coupen id from form
            valid_coupon = get_object_or_404(
                Coupons, name=coupon_name
            )  # get the corresponding coupon
        except:
        ...
        user_coupons = UserCoupons.objects.filter(
            user=request.user
        )  # get user all applied coupon if any
        if not UserCoupons.objects.filter(
            user=request.user, coupon=valid_coupon
        ).exists():
            user_coupon = UserCoupons.objects.create(
                coupon=valid_coupon, user=request.user, applied=True, is_active=True
            )
            user_coupon.save
            status = True
        else:
            user_coupon = get_object_or_404(
                UserCoupons, coupon=valid_coupon, user=request.user
            )
            if user_coupon.is_active:
                ...
                try:
                    ...
                    for cart_item in cart_items:
                        price = (
                            cart_item.variations.offer_price
                            if cart_item.variations.offer_price is not None
                            else cart_item.variations.price
                        )
                        total += price * cart_item.quantity
                        quantity += cart_item.quantity
                        try:
                            original_total += (
                                cart_item.variations.price * cart_item.quantity
                            )
                            offer_discount += (
                                cart_item.variations.price
                                - cart_item.variations.offer_price
                            ) * cart_item.quantity
                        except:
                            pass
        ...
        try:
            ...
            for cart_item in cart_items:
                ...
                try:
                    ...
                    offer_discount += (
                        cart_item.variations.price - cart_item.variations.offer_price
                    ) * cart_item.quantity
                except:
                    pass
            ...
            if valid_coupon:
                ...
                if total >= coupon_mv:
                    ...
                    if coupon_max_discount_value <= discount_value:
                        ...
                        messages.success(
                            request,
                            f"Maximum discount is {coupon_mv}",
                            extra_tags="coupon",
                        )
                ...
                # coupon apply minium value doesnt meet
                    ...
                    messages.error(
                        request,
                        f"Minium purchase value for this coupon is {coupon_mv}",
                        extra_tags="coupon",
                    )
                    total = round(total, 2)
                    tax = (total * 18) / 100
    ...
    if request.method == "GET":
        ...
        if original_total >= mn_vl:
            ...
            if offer_applied_discount > mx_vl:
                ...
                messages.success(
                    request, f"Maximum discount is {mx_vl}", extra_tags="com_offer"
                )
